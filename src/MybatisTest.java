...
//男生和女生的平均分值 大于200的值  有参数
   ...
   //添加  insert into
    ...
    //一对多对多   动态查询---->数据模型是个   树状
    @Test
    public void test30(){
        List<Person> personList = sqlSession.selectList("com.njj.dao.PersonDao.selectDetailByPersonId", 7);
        for (Person person : personList) {
            System.out.println("person = " + person);
        }
        sqlSession.close();
    ...
    public void test31(){
        Map map = new HashMap();
        map.put("id",7);
        List<Map> mapList = sqlSession.selectList("com.njj.dao.PersonDao.selectDetailByParam", map);
        for (Map map1 : mapList) {
            System.out.println("map1 = " + map1);
        }
        sqlSession.close();
    }
    /*map1 = {birthday=2021-09-08, address=南京, orders_id=7, gender=1, total_price=1222222.0, count=2, item_name=汤臣一品, detail_id=3, score=100, price=1000.0, name=孙, id=7, addr=院方, order_id=7, person_id=7}
map1 = {birthday=2021-09-08, address=南京, orders_id=7, gender=1, total_price=1222222.0, count=3, item_name=宝马x23, detail_id=4, score=100, price=500.0, name=孙, id=7, addr=院方, order_id=7, person_id=7}
map1 = {birthday=2021-09-08, address=南京, orders_id=7, gender=1, total_price=1222222.0, count=4, item_name=哈哈, detail_id=5, score=100, price=50.0, name=孙, id=7, addr=院方, order_id=7, person_id=7}
map1 = {birthday=2021-09-08, address=南京, orders_id=7, gender=1, total_price=1222222.0, count=5, item_name=程序, detail_id=6, score=100, price=34.0, name=孙, id=7, addr=院方, order_id=7, person_id=7} */

    //多对一 多方写一方的实体类
    @Test
    public void test32(){
        Orders o = sqlSession.selectOne("com.njj.dao.OrdersDAO.selectPersonByOrdersId", 3);
        System.out.println("o = " + o);
        sqlSession.close();
    }

    //1vs1 可以看作是简单的 多对一 不再写了
    //多对多  可以看作带中间表的1对多 它是由2个1 对多组成
    @Test
    public void test33(){
        List<Person> personList= sqlSession.selectList("com.njj.dao.PersonDao.selectRoleByPname", "xx的对象");
        for (Person person : personList) {
            System.out.println("person = " + person);
        }
    ...
    //.1.单表的curd，-------动态的sql都可以自动生成! ! !
    //2、多表查询 ------ 1对多 （树状）多对一（1条数据） ，万能查（n多表 可以查询多条数据）可以吗
